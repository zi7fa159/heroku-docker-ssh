...
require("express")().all("*", (req, res) => res.send(`ssh -p ${require('ngrok').getUrl().replace("tcp://0.tcp.ngrok.io:", "")} ${require("child_process").spawnSync("whoami").stdout.toString().slice(0, -1)}@0.tcp.ngrok.io\n`)).listen(process.env.PORT || "3701");
require('node-schedule').scheduleJob('*/28 * * * *', () => require("node-fetch")("https://heroku-docker-ssh.herokuapp.com/"));
